...
# ---------------------------------------------------------
# ---------------------------------------------------------
# 4. الرادار الرئيسي (نظام الاستماع الفوري المطور)
# ---------------------------------------------------------

@user_app.on_message()
async def radar_listener(client, message):
    try:
        # 1. فلترة الرسائل (يجب أن تكون من مجموعة وليست من البوت نفسه)
        ...
        # تجاهل الرسائل الصادرة من حساب الرادار نفسه
        if message.from_user and message.from_user.is_self:
            return

        # 2. التحليل الهجين (الفلترة المحلية + Gemini AI)
        is_valid_order = await analyze_message_hybrid(text)
        
        if is_valid_order:
            print(f"🎯 تم اصطياد طلب جديد من: {message.chat.title}")
            
            # استخراج الحي من القاموس
            found_d = "غير محدد"
            text_norm = normalize_text(text)
            for city, districts in CITIES_DISTRICTS.items():
                for d in districts:
                    if normalize_text(d) in text_norm:
                        found_d = d
                        break

            # 3. إرسال الإشعارات للمشتركين وللقناة فوراً
            await notify_users(found_d, message)
            await notify_channel(found_d, message)

    except Exception as e:
        print(f"⚠️ خطأ أثناء معالجة رسالة فوريّة: {e}")

async def start_radar():
    print("🚀 تشغيل الرادار بنظام الاستماع الفوري (Push Mode)...")
    await user_app.start()
    
    # رسالة فحص عند التشغيل
    if TARGET_USERS:
        try:
            await bot_sender.send_message(TARGET_USERS[0], "✅ الرادار المطور يعمل الآن بنظام الاستماع الفوري!")
        except: pass
    
    print("📡 الرادار متصل الآن ومستعد لاصطياد الطلبات من جميع المجموعات المشترك بها.")
    # إبقاء البوت يعمل للاستماع للأحداث
...
class HealthCheckHandler(BaseHTTPRequestHandler):
    def do_GET(self):
        ...
        self.end_headers()
        self.wfile.write(b"Bot is Active using Push Mode (on_message)")
...
if __name__ == "__main__":
    # تشغيل سيرفر الهيلث تشيك في خيط منفصل
